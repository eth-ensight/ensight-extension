...
export default function App() {
  ...
  useEffect(() => {
    const load = async () => {
      const res = await browser.storage.local.get("ensight:activeSession");
    
      const s = res["ensight:activeSession"] as Session | undefined;
      setSession(s ?? null);
    };
    ...
    const onChanged = (changes: any, area: string) => {
      ...
      const change = changes["ensight:activeSession"];
      if (!change) return;
    
      setSession((change.newValue as Session | undefined) ?? null);
    };
